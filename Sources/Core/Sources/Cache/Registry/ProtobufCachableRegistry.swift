...
public protocol ProtobufCachableRegistry: CachableRegistry, ProtobufCachable {
  /// - Returns: File to cache the registry to. Should just be a file name ending in ".bin" (e.g. "blocks.bin").
  ///            It's a function so that `ProtobufCachableRegistry` can be implemented in extensions.
