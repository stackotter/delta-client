...
public enum MeshError: LocalizedError {
  ...
  case mustUpdateBuffersBeforeRender
}
...
public struct Mesh {
  ...
  /// - Parameter encoder: Render encode to encode commands into.
  public func render(into encoder: MTLRenderCommandEncoder) throws {
    if isEmpty {
      return
    }
    ...
    guard
      vertexBufferIsValid,
      indexBufferIsValid,
      uniformsBufferIsValid,
      let vertexBuffer = vertexBuffer,
      let indexBuffer = indexBuffer,
      let uniformsBuffer = uniformsBuffer
    else {
      throw MeshError.mustUpdateBuffersBeforeRender
    }
