...
public struct WorldMesh {
  ...
  /// to the time they were queued which means that the ordering is more likely to be valid. For
  /// example, if the size of the job queue was not limited and the player turns around while
  /// sections are being prepared, all of the sections that were previously visible would be
  /// prepared before the ones that were actually visible.
  ...
  public mutating func addChunk(at position: ChunkPosition) {
    ...
    // The chunks required to prepare this chunk is the same as the chunks that require this chunk
    // to prepare. Adding this chunk may have made some of the chunks that require it preparable so
    // here we check if any of those can now by prepared. `chunksRequiredToPrepare` includes the
    // chunk itself as well.
  ...
  ///
  /// This method has an issue where any chunk already queued to be prepared will still be prepared
  /// and stored. However, it will not be rendered, so this isn't much of an issue. And it probably
  ...
  /// Collects any recently updated chunk section meshes and adds them to the mesh.
  public mutating func collectUpdatedMeshes(device: MTLDevice, commandQueue: MTLCommandQueue) throws {
    lock.acquireWriteLock()
    defer { lock.unlock() }

    let updatedMeshes = meshWorker.getUpdatedMeshes()
    for (position, var mesh) in updatedMeshes {
      try mesh.updateBuffers(device: device, commandQueue: commandQueue)
  ...
  /// - Parameters:
  ///   - shouldReverseOrder: If `true`, the sections will be mutated from furthest to closest.
  ///   - acquireLock: If `false`, a lock is not acquired, this is unsafe unless you already have a
  ///     lock.
  ///   - action: Action to perform on each visible mesh.
  public mutating func mutateEachVisibleMesh(
  ...
  public func accessMeshes(
    _ action: (
      _ meshes: [ChunkSectionPosition: ChunkSectionMesh],
      _ visibleSections: [ChunkSectionPosition]
    ) throws -> Void
  ) rethrows {
    lock.acquireWriteLock()
    defer { lock.unlock() }

    try action(meshes, visibleSections)
  }
  ...
  public mutating func sectionsAffectedBySectionUpdate(
    at position: ChunkSectionPosition,
    onlyLighting: Bool = false
