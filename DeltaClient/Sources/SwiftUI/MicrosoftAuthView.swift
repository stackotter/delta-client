          ...
          MicrosoftAuth.getXBoxLiveToken(microsoftToken, onCompletion: { xboxLiveToken, userHash in
            MicrosoftAuth.getXSTSToken(xboxLiveToken, onCompletion: { xstsToken in
              MicrosoftAuth.getMinecraftAccessToken(xstsToken, userHash, onCompletion: { minecraftAccessToken in
                ...
                DeltaClientApp.triggerError("Failed to get minecraft access token from xsts token: \(error)")
              })
            }, onFailure: { error in
                ...
                switch error {
                  case .xstsAuthenticationFailed(let xstsError):
                    switch xstsError.code {
                      ...
                      default:
                        DeltaClientApp.triggerError("Unknown XSTS token error: \(error)")
                    }
                  default:
                    DeltaClientApp.triggerError("Unknown XSTS token error: \(error)")
                }
              } else {
                DeltaClientApp.triggerError("Failed to get xsts token from xbox live token: \(error)")
              }
            })
          }, onFailure: { error in
            DeltaClientApp.triggerError("Failed to get xbox live token from microsoft access token: \(error)")
          })
        }, onFailure: { error in
          DeltaClientApp.triggerError("Failed to get microsoft access token: \(error)")
        })
      } catch {
        DeltaClientApp.triggerError("Authorization failed, redirectURL: \(url), error: \(error)")
      }
    }
  }
